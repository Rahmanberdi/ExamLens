...
import { adminApi, type Question } from '../../api/admin';
...
export function AdminQuestionEditor() {
  ...
  useEffect(() => {
    if (existing) {
      setExamId(String(existing.exam));
      setQNumber(String(existing.question_number));
      setQType(existing.question_type);
      setContent(existing.content);
      setMaxScore(existing.max_score);
      if (existing.question_type === 'single_choice' || existing.question_type === 'multiple_choice') {
        setOptions(existing.options as Record<string, string>);
      }
      const ca = existing.correct_answer;
      if (existing.question_type === 'single_choice') setSingleAnswer(ca[0] as string);
      else if (existing.question_type === 'multiple_choice') setMultiAnswers(ca as string[]);
      else if (existing.question_type === 'true_false') setTfAnswer(ca[0] as boolean);
      else if (existing.question_type === 'fill_blank') setFillAnswer(ca[0] as string);
    }
  }, [existing]);

  const buildPayload = (): Omit<Question, 'id'> => {
    let correct_answer: (string | boolean)[] = [];
    let opts: Record<string, string> = {};
    if (qType === 'single_choice') { correct_answer = [singleAnswer]; opts = options; }
    else if (qType === 'multiple_choice') { correct_answer = [...multiAnswers].sort(); opts = options; }
    else if (qType === 'true_false') { correct_answer = [tfAnswer]; }
    else { correct_answer = [fillAnswer]; }
    return {
      exam: Number(examId),
  ...
  const mutation = useMutation({
    mutationFn: () => isEdit ? adminApi.updateQuestion(Number(id), buildPayload()) : adminApi.createQuestion(buildPayload()),
    onSuccess: () => {
      ...
      <div style={{ maxWidth: 560, display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ display: 'flex', gap: 12 }}>
          ...
          <FormField label={t('questionNumber')}>
            <input type="number" style={{ ...inputStyle, width: 80 }} value={qNumber} onChange={(e) => setQNumber(e.target.value)} />
          </FormField>
        </div>
          ...
          <div>
            ...
            {OPTION_KEYS.map((key) => (
              <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                <span style={{ fontSize: 12, fontFamily: "'JetBrains Mono', monospace", width: 16, color: 'var(--ink-3)' }}>{key}</span>
                <input
        ...
        <FormField label={t('maxScore')}>
          <input type="number" step="0.5" style={{ ...inputStyle, width: 120 }} value={maxScore} onChange={(e) => setMaxScore(e.target.value)} />
        </FormField>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={() => navigate('/admin/questions')} style={{ padding: '6px 16px', border: '1px solid var(--line-2)', fontSize: 13, cursor: 'pointer', color: 'var(--ink-2)' }}>
            {t('cancel')}
          </button>
        </div>
      </div>

      <EndpointFooter method={isEdit ? 'PUT' : 'POST'} path={isEdit ? `/api/admin/questions/${id}/` : '/api/admin/questions/'} />
    </Shell>
  );
}
